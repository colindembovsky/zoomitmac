...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func handleCustomizeShortcutsMenuItem() {
        if shortcutCustomizationWindowController == nil {
            let controller = ShortcutCustomizationWindowController()
            controller.onShortcutsChanged = { [weak self] in
                self?.hotkeyManager.restart()
                self?.updateToggleMenuItemTitle()
            }
            shortcutCustomizationWindowController = controller
        }

        shortcutCustomizationWindowController?.show()
    }
